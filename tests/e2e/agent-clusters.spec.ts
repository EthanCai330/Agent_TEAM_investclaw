...
test.describe('InvestClaw Agent Cluster', () => {
  test('creates a cluster from a new task and opens the concise workflow view', async ({ page }) => {
    ...
    await expect(page.getByTestId('agent-cluster-workflow-summary')).toBeVisible();
    ...
    await page.getByRole('button', { name: '编辑编排' }).click();
    ...
    await expect(page.getByRole('button', { name: '保存为模板' })).toHaveCount(0);
    await expect(page.getByRole('button', { name: '添加 JS Workflow' })).toHaveCount(0);
    ...
    await expect(page.getByText(/启动中|运行中|Agent 子会话/).first()).toBeVisible();
  });
