...
export type AgentClusterRuntimeFailureKind = 'runtime_error' | 'incomplete_output' | 'stale_after_restart';
export type AgentClusterChildPromptMode = 'initial' | 'continuation' | 'repair';
export type AgentClusterCreationStageStatus = 'pending' | 'running' | 'completed' | 'error';
export type AgentClusterCreationStatusValue = 'running' | 'completed' | 'error' | 'aborted';
...
export type AgentClusterRuntimeMode = 'project_directory' | 'dynamic_harness';
...
export type WorkflowGateKind = 'completion' | 'artifact' | 'count' | 'schema' | 'llm';
export type WorkflowHumanGateDecision = 'approve' | 'reject';
export type WorkflowEdgeCondition = 'always' | 'on_pass' | 'on_fail' | 'on_revise';
export type WorkflowFanOutMode = 'fixed' | 'from_output';
export type InteractionMode = 'ask' | 'plan' | 'run' | 'review';
...
export type RuntimeArtifactMatchPolicy = 'literal' | 'glob' | 'one_or_more';
...
export type RuntimeArtifactRef = string | RuntimeArtifactSpec;
...
export interface AgentClusterManagerAgentPromptUpdate extends AgentClusterManagerAgentDraft {
...
export interface AgentClusterWorkflowTemplate {
  templateId: string;
  name: string;
  description?: string;
...
export interface AgentClusterWorkflowScript {
  source: string;
  sourceType: 'planner' | 'project' | 'user';
  version: number;
  hash: string;
  argsSchema?: Record<string, unknown>;
  createdAt: string;
...
export interface StartAgentClusterRunRequest {
  args?: Record<string, unknown>;
  templateId?: string;
  interactionMode?: InteractionMode;
}
